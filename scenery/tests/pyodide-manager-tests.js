import { assert, assertEquals, assertContains, assertInstanceOf, logTestStart, logTestPass, logTestFail } from './test-utils.js';
...
missive({
    "isolation_var": isolation_var,
    "shared_name": shared_name,
    "namespace_specific": namespace_specific
})
`,
                namespaceA
            );
...
print(f"Got age: {age}")
future_age = age + 10
print(f"Hello {name}! In 10 years you'll be {future_age} years old.")
print("Input test completed!")
`;

            const result = await manager.executeAsync("input_test", code);

            assert(!result.error, "Input handling should not have errors");
            assertContains(result.stdout, "Starting input test...", "Should start input test");
            assertContains(result.stdout, "Got name: Alice", "Should get correct name");
            assertContains(result.stdout, "Got age: 25", "Should get correct age");
            assertContains(result.stdout, "Hello Alice! In 10 years you'll be 35 years old.", "Should calculate correctly");
            assertContains(result.stdout, "Input test completed!", "Should complete input test");
...
`;
            ...
            const result = await manager.executeAsync("matplotlib_test", matplotlibCode);
