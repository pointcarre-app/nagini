/**
 * Nagini executions :: page logic
 *
 * One shared Pyodide manager for the whole page, created lazily on the
 * first run and reused everywhere (the namespace demo only changes the
 * namespace argument, not the manager). Every python-produced string is
...
const PACKAGES = ['matplotlib']; // needed by #matplotlib only, loaded at boot
const MICROPIP_PACKAGES = [];
const DEFAULT_TIMEOUT_MS = 60000;
const BOOT_TIMEOUT_MS = 180000;
const DEFAULT_FS_CONTENT = 'hello from the page side of the filesystem';
...
function renderResult(ctx, result) {
  ...
  if (ctx.step.showDump) {
    ctx.out.dump.pre.textContent = JSON.stringify({
      filename: result.filename,
      time: result.time,
      stdout: result.stdout,
      stderr: result.stderr,
      error: result.error,
      missive: result.missive,
      figures: (result.figures || []).length + ' figure(s)',
    }, null, 2);
    show(ctx, 'dump');
  }
...
async function runStep(demo, ctx, button) {
  ...
  try {
    ...
    try {
      ...
      if (demo.fsDemo) {
        const content = ctx.fsField.value;
        if (!(await m.fs('exists', { path: '/data' }))) {
          await m.fs('mkdir', { path: '/data' });
        }
        await m.fs('writeFile', { path: '/data/notes.txt', content }, 10000);
      }
...
async function runJoint(demo, ctxs, button) {
  button.disabled = true;
  ctxs.forEach(resetOutputs);
  try {
    const m = await ensureManager();
    beginRun(demo.id);
    try {
      // both promises exist before either resolves: this is the point
      const results = await Promise.all(ctxs.map((ctx) => m.executeAsync(
        ctx.step.file,
        ctx.editor.getValue(),
        ctx.step.namespace,
        ctx.step.timeoutMs ?? DEFAULT_TIMEOUT_MS
      )));
      results.forEach((result, i) => renderResult(ctxs[i], result));
    } finally {
      endRun();
    }
  } catch (e) {
    ctxs.forEach((ctx) => renderRejection(ctx, e));
  } finally {
    button.disabled = false;
  }
}
...
function buildStep(demo, step) {
  ...
  const out = {
    ...
    dump: outputPanel('result object', 'out-dump'),
    error: outputPanel('result.error', 'out-error'),
    stderr: outputPanel('stderr', 'out-stderr'),
    rejected: outputPanel(
      step.expectReject ? 'rejected promise :: expected here' : 'rejected promise',
      'out-rejected'
    ),
  };
  for (const key of ['stdout', 'missiveRaw', 'missive', 'dump', 'error', 'stderr', 'rejected']) {
  ...
  // run bar (joint demos get one shared bar in buildDemo instead)
  const runbar = el('div', 'runbar');
  if (demo.fsDemo) {
    const fsLabel = el('label', 'fs-label', 'file content');
    const fsField = document.createElement('input');
    fsField.type = 'text';
    fsField.className = 'fs-field';
    fsField.value = DEFAULT_FS_CONTENT;
    fsLabel.htmlFor = fsField.id = 'fs-content-' + demo.id;
    ctx.fsField = fsField;
    runbar.appendChild(fsLabel);
    runbar.appendChild(fsField);
  }
  if (!demo.joint) {
...
function buildDemo(demo) {
  ...
  if (demo.joint) {
    const runbar = el('div', 'runbar');
    const button = el('button', 'btn btn-sm btn-exec', demo.joint);
    button.addEventListener('click', () => runJoint(demo, ctxs, button));
    runbar.appendChild(button);
    wrap.appendChild(runbar);
  }
